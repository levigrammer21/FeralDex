...
function render(){
 ...
 $('#feralCount').textContent=fs.length+'/'+db.ferals.length; $('#itemCount').textContent=its.length; $('#locCount').textContent=ls.length;
 $('#feed').innerHTML=[...fs.slice(-3).map(x=>`<div class="log"><b>FERAL RECORD // ${x.name.toUpperCase()}</b><span>Roster data available from Chapter ${x.chapter}</span></div>`),...its.slice(-2).map(x=>`<div class="log"><b>ITEM RECORD // ${x.name.toUpperCase()}</b><span>System inventory data acquired</span></div>`)].reverse().join('')||'<div class="log"><b>INITIALIZING ARCHIVE</b><span>Awaiting System data...</span></div>';
 renderDex(); renderItems(); renderMap();
}
function renderDex(){let q=($('#search')?.value||'').toLowerCase(); let list=db.ferals.filter(x=>x.name.toLowerCase().includes(q)||x.type.toLowerCase().includes(q)||x.feralClass.toLowerCase().includes(q)); $('#rosterTotal').textContent=`${db.ferals.length} ROSTER SLOTS`; $('#feralGrid').innerHTML=list.map(x=>visible(x)?`<article class="panel card feralCard" data-feral="${x.id}"><small>DEX // CH ${x.chapter}</small><h3>${x.name}</h3><p class="flavorMini">${x.flavor}</p><div class="tags"><span class="tag">${x.type}</span><span class="tag">${x.feralClass}</span></div><div class="meta">${x.level?'LEVEL '+x.level:'LEVEL DATA VARIABLE'} • ${x.role}</div></article>`:`<article class="panel card locked"><small>UNDISCOVERED // CHAPTER ${x.chapter}</small><h3>UNKNOWN FERAL</h3><div class="silhouette">?</div><div class="tags"><span class="tag">DATA SEALED</span></div></article>`).join(''); $$('[data-feral]').forEach(e=>e.onclick=()=>openFeral(e.dataset.feral))}
...
function openFeral(id){let x=db.ferals.find(f=>f.id===id); let abs=(x.abilities||[]).length?x.abilities.map(a=>`<span class="ability">${a}</span>`).join(''):'<span class="muted">No named abilities archived at first encounter.</span>'; $('#modalBody').innerHTML=`<small>SYSTEM DEX // FERAL RECORD</small><h2>${x.name}</h2><div class="scanline"></div><p class="dexFlavor">${x.flavor}</p><div class="statgrid"><div><small>HEIGHT*</small><b>${x.height}</b></div><div><small>WEIGHT*</small><b>${x.weight}</b></div><div><small>CLASS</small><b>${x.feralClass}</b></div><div><small>LEVEL</small><b>${x.level||'Variable'}</b></div><div><small>THREAT*</small><b>${x.threat}</b></div><div><small>ROLE*</small><b>${x.role}</b></div></div><p><b>TYPE</b><br>${x.type}</p><p><b>TEMPERAMENT</b><br>${x.temperament||'Not recorded'}</p><p><b>LIKELY HABITAT*</b><br>${x.habitat}</p><p><b>KNOWN ABILITIES</b></p><div class="abilities">${abs}</div><p><b>ARCHIVE NOTES</b><br>${x.notes}</p><small>* DEX ESTIMATE / FLAVOR STAT • FIRST ARCHIVED // BOOK 01 • CHAPTER ${x.chapter}</small>`; $('#modal').classList.add('open')}
